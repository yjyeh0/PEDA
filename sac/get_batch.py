...
class GetBatch():
    ...
    def __call__(self):
        ...
        s, pref, a, raw_r, n_s, dones = [], [], [], [], [], []
        for i in batch_inds:
            traj = self.trajectories[int(self.sorted_inds[i])]
            # randomly get the starting idx
            step = random.randint(0, traj['rewards'].shape[0] - 1)
            s.append(traj['observations'][step].reshape(1, -1, self.state_dim))
            n_s.append(traj['next_observations'][step].reshape(1, -1, self.state_dim))
            trj_len = traj['observations'].shape[0]
            dones.append(int(traj["terminals"][step]))

            # if step == (trj_len - 1):  #done
            #     n_s.append(traj['observations'][step].reshape(1, -1, self.state_dim))
            #
            # else:
            #     n_s.append(traj['observations'][step+1].reshape(1, -1, self.state_dim))

            a.append(np.maximum(np.minimum(traj['actions'][step].reshape(1, -1, self.act_dim), self.act_high), self.act_low) / self.act_high) # assume scale if relflective to 0 (-x, x)
            raw_r_to_add = traj['raw_rewards'][step].reshape(1, -1, self.pref_dim)
            raw_r.append(raw_r_to_add)
            pref.append(traj['preference'][step].reshape(1, -1, self.pref_dim))
        ...
        # n_s = np.clip((n_s - self.state_mean) / self.state_std, -10, 10)
        n_s = torch.from_numpy(np.concatenate(n_s, axis=0)).to(dtype=torch.float32, device=self.device)

        dones = torch.from_numpy(np.array(dones)).to(dtype=torch.float32, device=self.device)
        ...
        s = torch.squeeze(s)
        pref = torch.squeeze(pref)
        a = torch.squeeze(a)
        raw_r = torch.squeeze(raw_r)
        n_s = torch.squeeze(n_s)
        dones = dones.unsqueeze(-1)

        return s, pref, a, raw_r, n_s, dones
