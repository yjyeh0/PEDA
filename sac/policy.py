...
import copy
import random
from sac.model import TwinnedQNetwork, GaussianPolicy
from torch.optim import Adam
# from torch.utils.tensorboard import SummaryWriter
# from rltorch.memory import MultiStepMemory, PrioritizedMemory
from sac.base import QMemory
from sac.utils import grad_false, hard_update, soft_update,\
    update_params #, to_batch, RunningMeanStats
import random
...
#         self.vis = visdom.Visdom(env=f'MOSAC{datetime.now().strftime("%m%d")}-{env_name}_pref{set_num}_buf{buf_num}',
#                                  port=8097)
#         self.spec = spec
#         if spec['pref'][0] == 0.9:
#             print(999)
...
#                                                         ylabel='Reward value',
#                                                         title='Value Dynamics ' + str(self.spec['pref']) + ' ' + str(
#                                                             self.spec['seed']),
...
class SacAgent():
    ...
    def __init__(self, env, log_dir, num_steps=3000000, batch_size=256,
        ...
        self.reward_num = self.env.obj_dim
        ...
        self.model_dir = os.path.join(log_dir, 'ckpt')
        ...
        self.tot_t = []
        self.reward_v = []
        if self.reward_num == 3:
            PREF_ = np.load("3pref_table.npy")
        elif self.reward_num == 4:
            PREF_ = np.load("4pref_table.npy")
        elif self.reward_num == 5:
    ...
    def get_pref(self):
        preference = np.random.rand(self.reward_num)
    ...
    # batch: s, pref, a, raw_r, n_s, dones
...
#    def learn(self):
        ...
        # yjyeh: there is problem, why is it random?
        PREF_SET = []
    ...
    def calc_policy_loss(self, batch, weights, preference, PREF):
        ...
        for a, c in enumerate([self.critic] + self.Q_memory.sample()):  # Use critic from Q Replay Buffer
            for b, i in enumerate(PREF):  # Get Q from preference set W
                p_batch = torch.tensor(i, device=self.device).clone().detach().repeat(self.batch_size, 1)
    ...
    def save_models(self, num):
        self.policy.save1(os.path.join(self.model_dir, 'policy_' + str(num) + '.pth'), self.policy_optim)
        self.critic.save2(os.path.join(self.model_dir, 'critic_' + str(num) + '.pth'), self.q1_optim,  self.q2_optim)
        self.critic_target.save(os.path.join(self.model_dir, 'critic_target.pth'))

    def load_models(self, num):
        self.policy.load1(os.path.join(self.model_dir, 'policy_' + str(num) + '.pth'), self.policy_optim)
        self.critic.load2(os.path.join(self.model_dir, 'critic_' + str(num) + '.pth'), self.q1_optim,  self.q2_optim)
        self.critic_target.load(os.path.join(self.model_dir, 'critic_target.pth'))
